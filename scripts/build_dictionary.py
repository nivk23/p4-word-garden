...
# Definitions written from Merriam-Webster's Elementary Dictionary: our own wording at
# their register and, crucially, their choice of sense. MW's own sentences are never stored
# (their terms forbid reproducing their content) — scripts/fetch_mw.py keeps the raw
# responses in a cache outside the repo, and these are written from them.
MW_DERIVED_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "definitions_mw.json")
MW_DERIVED = {}
if os.path.exists(MW_DERIVED_PATH):
    MW_DERIVED = {k.lower(): v for k, v in json.load(open(MW_DERIVED_PATH, encoding="utf-8")).items()}
...
def main():
    ...
    definitions, report = {}, {"mw_derived": [], "manual": [], "no_entry": [], "weak_match": []}

    for entry in entries:
        key = entry["word"].lower()
        if key in MW_DERIVED:
            definitions[key] = MW_DERIVED[key]
            report["mw_derived"].append(entry["word"])
            continue
        if key in MANUAL:
    ...
    out = [
        ...
        " *",
        " * Sources: where we have covered the word, the definition is written from Merriam-",
        " * Webster's Elementary Dictionary (the sense and register an 8-11 year old's dictionary",
        " * uses), simplified for a Primary 4 reader. No Merriam-Webster text is reproduced here —",
        " * their terms forbid it — so those entries are our own wording of the sense their",
        " * dictionary gives. Everything else comes from Princeton WordNet 3.1, which is",
        " * distributed under the WordNet Licence:",
    ...
    wordnet_count = len(definitions) - len(report["manual"]) - len(report["mw_derived"])
    print(f"words: {len(entries)}  defined: {len(definitions)}  "
          f"(from Merriam-Webster {len(report['mw_derived'])}, WordNet {wordnet_count}, "
          f"written here {len(report['manual'])})")
